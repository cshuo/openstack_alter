...
from .db.utils import create_vm_table
...
class ServersController(wsgi.Controller):
	...
	#    LOG.info('test1: ' + str(inst.uuid) + str(inst.hostname) + str(inst.host) + str(inst.display_name))
	if 'app_type' in server_dict:
	    create_vm_table()
	    dbu = DbUtil()
	    for inst in instances:
                dbu.add_vm(str(inst.uuid), str(inst.hostname), server_dict['app_type'])
